...
jQuery._tfWidget = function(container, callback) {
              ...
              '<div id="tf-picker-and-buttons">'+
                ...
                '<table id="tf-buttons">'+
                  ...
                  '<tr>'+
                    ...
                    '<input type="file" id="load-tf-file" style="display:none;"/>'+
                  '</tr>'+
                '</table>'+
              '</div>'+
            '</div>';
  ...
  function saveTFFile(e) {
    writeToFile(controlPoints);
  }

  function readTFFile(e) {
    ...
    reader.onload = function(e) {
      ...
      try {
        var tmpJson = JSON.parse(contents);
      }
      catch (err) {
  ...
  function writeToFile(controlPoints) {
    var str = JSON.stringify(controlPoints);
    var pom = document.createElement('a');
    pom.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(str));
    pom.setAttribute('download', 'tf');
  ...
  var controlPointsToArray = function() {
    ...
    for (var i = 0; i < 256; i ++) {
      ...
      arr.push(Math.floor(tmpAlpha.y * 256));
    }
    return arr;
  }
